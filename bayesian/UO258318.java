 ...
 * 
 * P( Dyspnoea?=no |  Has lung cancer=no Tuberculosis or cancer=yes)
 * 
 * @author 
...
public class UO258318 {
	...
	/**
	 * Calculates an Likelihood Weighting Inference for a given evidence and
	 ...
	 * "Has lung cancer" = no and "Tuberculosis or cancer" = yes.
	 * 
	 * @return
	...
	private EvidenceCase getEvidence() {
		...
		try {
			...
			evidence.addFinding(getProbNet(), lungCancer.getName(), lungCancer.getState("no").getName());
			// Tuberculosis or cancer = YES
			Variable tuberculosisOrCancer = getProbNet().getVariable("Tuberculosis or cancer");
			evidence.addFinding(getProbNet(), tuberculosisOrCancer.getName(),
					tuberculosisOrCancer.getState("yes").getName());
	 ...
	 * only one variable of interest. That variable is "Dysphonea?".
	 * 
	 * @return
	...
	private List<Variable> getVariablesOfInterest() {
		...
		try {
			Variable dysphonea = getProbNet().getVariable("Dyspnoea?");
			variablesOfInterest.add(dysphonea);
		} catch (NodeNotFoundException e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) throws Exception {
		...
		// The Likelihood Weighting Inference is calculated
		obj.LWInference(variablesOfInterest, evidence);
	}
